...
function reverseWords(str) {
  ...
  for (let i = 0; i < arrWords.length; i += 1) {
    result += arrWords[i].split('').reverse().join('');
...
function encodeToRot13(str) {
  const arrLetters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const arrLettersToRot13 =
    'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str
    .split('')
    .map((value) => {
      const index = arrLetters.indexOf(value);
      if (index !== -1) {
        return arrLettersToRot13[index];
      }
      return value;
    })
    .join('');
